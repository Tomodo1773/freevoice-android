...
class VoiceInputController(
    ...
    interface Recorder { fun start(); fun stop(): File?; fun cancel() }
    interface Gateway {
    ...
    fun start(targetPackage: String, validationError: String?): Long? = synchronized(lock) {
        ...
        generation++
        if (validationError != null) { failUserLocked(validationError); return null }
        cancelled = false
        packageName = targetPackage
        transitionLocked(VoiceInputState.Starting)
        try { recorder.start(); transitionLocked(VoiceInputState.Recording); generation }
        catch (e: Exception) { failLocked("録音を開始できなかった", e); null }
    }

    fun stop() = synchronized(lock) {
        if (state !is VoiceInputState.Recording || closed) return
        val wav = try { recorder.stop() } catch (e: Exception) { failLocked("録音を終了できなかった", e); return }
        if (wav == null || wav.length() <= 44L) { failUserLocked("音声が録音されていない"); return }
        ...
        executor.execute {
            try {
                ...
                if (isActive(jobGeneration)) { callbacks.committed(jobGeneration, text, jobPackage, formatted.fallback); transition(jobGeneration, VoiceInputState.Idle) }
            } catch (e: Exception) {
                if (isActive(jobGeneration)) fail(jobGeneration, classify(e), e)
            } finally { wav.delete() }
        }
    }
    ...
    fun close() = synchronized(lock) { closed = true; cancelLocked(); executor.shutdownNow() }
    fun currentState(): VoiceInputState = synchronized(lock) { state }

    private fun isActive(jobGeneration: Long) = synchronized(lock) { !cancelled && !closed && generation == jobGeneration }
    private fun transition(jobGeneration: Long, next: VoiceInputState) = synchronized(lock) { if (!cancelled && !closed && generation == jobGeneration) transitionLocked(next) }
    private fun transitionLocked(next: VoiceInputState) { state = next; callbacks.stateChanged(next) }
    private fun fail(jobGeneration: Long, message: String, error: Throwable) = synchronized(lock) { if (generation == jobGeneration) failLocked(message, error) }
    private fun failLocked(message: String, error: Throwable) { transitionLocked(VoiceInputState.Error((error as? UserVisibleException)?.message ?: message)); callbacks.failed(generation, message, error) }
    private fun failUserLocked(message: String) { transitionLocked(VoiceInputState.Error(message)); callbacks.failed(generation, message, null) }
    ...
    private fun classify(error: Throwable): String = when {
        error.message?.contains("HTTP 401") == true || error.message?.contains("HTTP 403") == true -> "API キーまたは権限を確認して"

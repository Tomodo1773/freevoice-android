...
「モデル」タブの LangSmith セクションで API キー・プロジェクト・リージョン（US/EU）を設定すると、音声入力の中身を LangSmith へ送信できる。後処理プロンプトの改善やデバッグに使う。既定はオフ。

1録音が1トレースになる。`recording` を親として、文字起こしと文章整形が子として並ぶ。

- 文字を入力できたときだけ、まとめて1回送る。無音・認識エラー・キャンセルで終わった録音は何も送らない。
- トレースの Input は文字起こしの生テキスト、Output は実際に入力欄へ挿入したテキスト。
- 所要時間には発話時間が含まれる。ストリーミング認識は喋っている間ずっと続くので、長い発話は長いトレースになる。
- 話題コンテキストの蒸留は入力確定後に録音の外で走るため、別のトレースとして届く。
